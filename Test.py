...
check(1, "a new account keeps its opening balance", 1000,
      lambda: bank_account("Mon", 1000).get_balance())
...
check(4, "the account prints owner and balance", "Mon: 1500.00",
      lambda: bank_account("Mon", 1500))
check(5, "overdrawing is refused", "refused",
      refuses(lambda: bank_account("Mon", 1000).withdraw(5000)))
check(6, "a negative deposit is refused", "refused",
      refuses(lambda: bank_account("Mon", 1000).deposit(-100)))
check(7, "a negative opening balance is refused", "refused",
      refuses(lambda: bank_account("Mon", -50)))
